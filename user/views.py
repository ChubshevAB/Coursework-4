...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            user = form.save()
            messages.success(
                request, "Регистрация успешна! Проверьте ваш email для подтверждения."
            )
...
class VerifyEmailView(View):
    def get(self, request, token):
        try:
            ...
            user.save()
            messages.success(
                request, "Email успешно подтверждён! Теперь вы можете войти."
            )
            return redirect("login")
...
class CustomPasswordResetConfirmView(PasswordResetConfirmView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, "Пароль успешно изменён! Теперь вы можете войти."
        )
        return super().form_valid(form)
