    ...
    def form_valid(self, form):
        ...
        if recipients:
            ...
            if invalid_recipients.exists():
                form.add_error(
                    "recipients", "Вы не можете использовать этих получателей"
                )
                return self.form_invalid(form)
    ...
    try:
        ...
        result = {
            ...
            "message": (
                ...
                if all_successful
                else "Рассылка завершена с ошибками"
            ),
        }
        ...
        return {"status": "error", "message": f"Рассылка с ID {mailing_id} не найдена"}
    except Exception as e:
...
def home_view(request):
    if request.user.is_authenticated:
        context = {
            ...
            "unique_recipients": Recipient.objects.filter(owner=request.user)
            .distinct()
            .count(),
        }
    else:
    ...
    def post(self, request, pk):
        try:
            ...
            if mailing.status == "completed":
                return JsonResponse(
                    {"status": "error", "message": "Рассылка уже завершена"}, status=400
                )
            ...
            return JsonResponse(
                {"status": "success", "message": "Рассылка успешно запущена"}
            )

        except Mailing.DoesNotExist:
            return JsonResponse(
                {"status": "error", "message": "Рассылка не найдена"}, status=404
            )
        except Exception as e:
